...
$userId = $_SESSION['userId']; // Change from employeeId to userId
$attendanceRecords = [];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
   ...
   $insertQuery = "INSERT INTO attendance (date, status, checkInTime, checkOutTime) VALUES ( ?, ?, ?, ?)";
   $stmt = $conn->prepare($insertQuery);
   $stmt->bind_param("ssss", $date, $status, $checkInTime, $checkOutTime);
   ...
   header("Location: test-attendance.php");
   exit();
}

$selectQuery = "SELECT date, checkInTime, checkOutTime, status FROM attendance WHERE attendanceId = ?";
